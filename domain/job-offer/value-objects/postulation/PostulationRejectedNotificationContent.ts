...
export default class PostulationNotificationContent implements IValueObject {
    ...
    public static create(content:string){
        if(content == null || content == undefined){
            throw new PostulationNotificationEmptyContentException('El contenido no puede estar vacio')
        }
        if(typeof content !="string"){
            throw new PostulationNotificationInvalidContentException('El contenido tiene que ser un string');
        }
        if(!content.trim()){
            throw new PostulationNotificationEmptyContentException('El contenido no puede estar vacio')  
        }
