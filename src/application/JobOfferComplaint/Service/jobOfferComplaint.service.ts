...
export class JobOfferComplaintService {
  ...
  async createjobOfferComplaint(
    complaint: JobOfferComplaint,
    idJobOffer: JobOfferId,
  ): Promise<any> {
    const JobOfferComplaint =
      this._jobOffermapper.convertDomainToDto(complaint);
    const JobOfferId =
      this._jobOffermapper.convertDomainToDtoCreateJobOffferid(idJobOffer);

    return await this._jobOfferPersistence.createjobOfferComplaint(
      JobOfferComplaint,
      JobOfferId,
    );
  }
  ...
  async updatejobOfferComplaint(complaint: JobOfferComplaint): Promise<any> {
    const JobOfferComplaint =
      this._jobOffermapper.convertDomainToDto(complaint);
    return await this._jobOfferPersistence.UpdateAceptedobOfferComplaint(
      JobOfferComplaint,
    );
  }
}
