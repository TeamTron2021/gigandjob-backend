...
import InterviewToSave from '../ports/InterviewToSave.dto';

export default class CreateInterviewService implements IAplicationService {
  execute(interviewDto: CreateInterviewDto): InterviewToSave {
    const mapperDtoToDomain: CreateInterviewMapper = new CreateInterviewMapper(
      interviewDto,
    );
    const interview: Interview<InterviewStatus> = mapperDtoToDomain.map();
    const interviewToSave: InterviewToSave = {
