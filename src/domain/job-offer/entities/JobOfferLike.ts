...
export class JobOfferLike {
  ...
  constructor(public id: JobOfferLikedId, private value: boolean) {}
    ...
    for (let x = 0; x <= object.length - 1; x++) {
      ...
      if (compare === compare2) {
        ...
        object[x].eventRecorder.push(
          new jobofferlike_event(object[x].id, object[x].value),
        );
        return object[x];
      }
    }
    throw new JobOfferWithoutLikeException('No hay likes a remover');
  }
}
