...
export class Notification<T> {
  ...
  getEvents(): NotificationEvents<T>[] {
    return this.events;
  }

  notifyUserRegistered() {
    this.events.push(
      new UserRegisteredNotificationSent<T>(
        this._ID,
        this.subject,
        this.content,
      ),
  ...
  notifyUserSuspended() {
    this.events.push(
      new UserSuspendedNotificationSent<T>(
        this._ID,
        this.subject,
        this.content,
      ),
  ...
  notifyUserReactivated() {
    this.events.push(
      new UserReactivatedNotificationSent<T>(
        this._ID,
        this.subject,
        this.content,
      ),
  ...
  notifyCVLoaded() {
    this.events.push(
      new CVLoadedNotificationSent<T>(this._ID, this.subject, this.content),
    );
  }

  notifyCVAproved() {
    this.events.push(
      new CVAprovedNotificationSent<T>(this._ID, this.subject, this.content),
    );
  }

  notifyCVRejected() {
    this.events.push(
      new CVRejectedNotificationSent<T>(this._ID, this.subject, this.content),
    );
  }
}
