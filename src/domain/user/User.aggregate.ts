...
type CVType<S> = S extends UserStatus.Unconfirmed
  ? CV<CVStatus.Unconfirmed> | CV<CVStatus.Rejected>
  : S extends UserStatus.Active
  ? CV<CVStatus.Aproved>
  : CV<CVStatus>;
...
export class User<S extends UserStatus> {
  ...
  confirm(this: User<UserStatus.Unconfirmed>): User<UserStatus.Active> {
    ...
    user.eventRecorder.push(new UserConfirmed(user.ID, user.status));
    return user;
  }

  suspend(
    this: User<UserStatus.Active | UserStatus.Unconfirmed>,
  ): User<UserStatus.Supended> {
    const user = new User(
      this.firstname,
      this.lastname,
      this.birthday,
    ...
    user.eventRecorder = this.eventRecorder.slice(0);
    user.eventRecorder.push(new UserSuspended(user.ID, user.status));
    return user;
  }
  ...
  reactive(this: User<UserStatus.Supended>): User<UserStatus.Active> {
    ...
    user.eventRecorder = this.eventRecorder.slice(0);
    user.eventRecorder.push(new UserReactivated(user.ID, user.status));
    return user;
  }
  ...
  approveCV(
    ...
    if (this.cv) {
      ...
      userActivated.eventRecorder = [
        ...userActivated.eventRecorder,
        ...cvApproved.getEvents(),
      ];
  ...
  rejectCV(this: User<UserStatus.Unconfirmed>) {
    if (this.cv) {
