...
describe('Testing companyMail value object',()=>{
    it('Should throw an empty companymail error', ()=>{
        const mail:any = null; 
        expect(()=> EmployeerCompanyMail.create(mail)).toThrow(new EmployeerEmptyCompanyMailException('El email del empleador no puede estar vacio'));
    })
    it('Should throw an invalid companyMail error',()=>{
        const mail:string = 'perez@com';
        expect(()=> EmployeerCompanyMail.create(mail)).toThrow(new EmployeerInvalidCompanyMailException(
