...
describe('Value Object JobOfferComplaintIssue', () =>{
	test('Should return a error: JobOfferComplaintIssue ',() =>{
		...
		expect(() => JobOfferComplaintIssue.create(issue)).toThrowError('La Issue de JobOfferComplaint no puede ser undefined o null');
	})
    ...
    test('Should return the happy path',() =>{
		...
		expect(JobOfferComplaintIssue.create(issue)).toBeInstanceOf(JobOfferComplaintIssue);
	})
})