import JobOfferEmptySkillException from '../../../../src/domain/job-offer/exceptions/JobOffer/JobOfferEmptySkillException';
...
describe('Testing value object JobOfferSkill', () => {
  it('Should thrown empty skill error', () => {
    expect(() => JobOfferSkill.create('')).toThrow(
      new JobOfferEmptySkillException('La habilidad no puede estar vacia'),
    );
  });
  it('Should thrown empty skill error', () => {
    const skill: any = null;
    expect(() => JobOfferSkill.create(skill)).toThrow(
      new JobOfferEmptySkillException('La habilidad no puede estar vacia'),
    );
  });
