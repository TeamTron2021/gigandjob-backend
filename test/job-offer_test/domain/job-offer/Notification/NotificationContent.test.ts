...
describe('Testing notification content value object', ()=>{
    test('Should throw an empty notification content error', ()=>{
        ...
        expect(()=>JobOfferNotificationContent.create(content)).toThrow(new NotificationEmptyContentException('El contenido no puede estar vacio'))
    });
    ...
    test('Should throw an empty notification content error', ()=>{
        const content: any = 8
        expect(()=>JobOfferNotificationContent.create(content)).toThrow(new NotificationInvalidContentException('El contenido tiene que ser un string'))
    });
